...
export const useAddDataToStore = <DataSchema>(): [(request: RequestPayload<DataSchema>) => Promise<void>, QueryState<DataSchema>] => {
  ...
  const runQuery = useCallback( (request: RequestPayload<DataSchema>): Promise<void> => {
    const responsePromise = queryClient.addDataToStore<DataSchema>(request.storeId)(request.data);
